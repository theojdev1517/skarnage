...
export function HostJoinApprovals({
  game,
  busy = false,
  onApprove,
  onDeny,
}: HostJoinApprovalsProps) {
  if (!game.pending_joins?.length) return null;
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/75">
      ...
      >
        <h2 className="text-lg font-semibold text-amber-300 mb-1">Seat requests</h2>
        <p className="text-zinc-400 text-sm mb-4">
          Approve players to sit at the table. Mid-hand joins wait for the next deal.
        </p>
        <ul className="space-y-3 max-h-64 overflow-y-auto">
          {game.pending_joins.map((req) => (
            <li
              key={req.id}
