...
export default function SkarneyHandStepper() {
  ...
  const nextStep = () => {
    ...
    let newGame = engine.advanceToNextPhase(game);
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white p-8 font-mono">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-zinc-400 mb-8">Proper deck tracking + betting enforcement</p>

        <div className="flex gap-4 mb-8 flex-wrap">
          <button 
            onClick={startNewHandFull} 
            className="bg-green-600 hover:bg-green-500 px-6 py-3 rounded font-semibold"
          >
            🚀 Start New Hand (Button + Blinds)
          </button>
          <button 
            onClick={startNewHand} 
            className="bg-white text-black px-6 py-3 rounded font-semibold hover:bg-zinc-200"
          >
            Legacy New Hand
          </button>
          <button 
          ...
          <button 
            onClick={reset} 
            className="border border-zinc-700 hover:bg-zinc-900 px-6 py-3 rounded"
          >
            Reset
          </button>
        </div>
  ...
  return (
    <div 
      ...
      className={`bg-zinc-900 border-2 p-4 rounded transition-all ${
        p.seat === game.current_player_seat 
    ...
    >
      <div className="flex justify-between">
        <strong>
          {p.display_name || `Player ${p.seat}`} (Seat {p.seat}) 
          {p.seat === game.current_player_seat && ' ← ACTING'}
        </strong>
        <span>Stack: ${p.stack}</span>
      </div>

      <div className="text-sm font-mono mt-3 space-y-1">
        <div>Original: {p.hole_cards.join(' ') || '(not dealt)'}</div>
          ...
          <button onClick={() => doBet(p.seat, "raise", 200)} className="text-xs bg-amber-900 hover:bg-amber-800 px-4 py-1 rounded">Raise $200</button>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-zinc-900 border border-zinc-700 p-6 rounded max-h-96 overflow-auto">
                <h3>Action Log</h3>
                {log.map((line, i) => (
                  <div key={`log-${i}`} className="text-sm py-0.5">{line}</div>
                ))}
              </div>
                          <div className="space-y-6">
              {/* ... existing Top Board ... */}
              ...
              {/* === TEMPORARY DECK DEBUG === */}
              {game && (
                <div className="bg-zinc-900 border border-zinc-700 p-4 rounded text-xs font-mono">
                  <h4 className="font-semibold mb-2 text-amber-400">🔍 Deck Debug</h4>
                  <div>Deck Size Remaining: <strong>{game.deck.length}</strong></div>
                  <div>Current Index: <strong>{game.deck_index}</strong></div>
                  <div>Cards Dealt This Hand: <strong>{game.deck_index}</strong></div>
                  <div className="mt-2 text-[10px] text-zinc-500 break-all">
                    Next 5 cards: {game.deck.slice(game.deck_index, game.deck_index + 5).join(' ')}
                  </div>
                </div>
              )}
            </div>
